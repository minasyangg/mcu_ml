...
class Op:
    ...
    def gradient_as_tuple(self, out_grad: "Value", node: "Value") -> Tuple["Value"]:
        ...
        # print(out_grad.shape, node.inputs[1].shape)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs) != 0:
            input_adj_grad = node.op.gradient_as_tuple(node.grad, node)
            for idx, nd in enumerate(node.inputs):
                if nd  is not None and nd in node_to_output_grads_list.keys():
                    node_to_output_grads_list[nd].append(input_adj_grad[idx])
                else:
                    try:
                        node_to_output_grads_list[nd] = [input_adj_grad[idx]]
                    except IndexError:
                        print('AD: Index out of Range')
                        break
		...
		for nd in (c1.inputs):
			if len(nd.inputs) != 0:
				...
				if nd not in visited:
					visited.append(nd)
				if nd in topo_order:
					topo_order.remove(nd)
		topo_order.remove(c1)
		visited.append(c1)
	return visited
...
def topo_sort_dfs(node, visited, topo_order):
	...
	if node.inputs not in visited:
		for nd in (node.inputs):
			if nd.inputs != 0 and nd not in visited:
				topo_sort_dfs(nd, visited, topo_order)
			else:
				if nd not in visited:
					visited.append(nd)
		if node not in visited and node in topo_order:
			visited.append(node)
